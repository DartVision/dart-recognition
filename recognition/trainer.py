...
class Trainer(object):
    ...
    def train_step(self, images, ground_truth):
        ...
        tf.summary.scalar('loss_train/total', loss,
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            if self.global_step.value() % 1 == 0:
                detect_loss, distance_loss, field_loss = self.evaluate()
                tf.summary.scalar('loss_eval/detect', detect_loss, step=self.global_step)
                tf.summary.scalar('loss_eval/distance', distance_loss, step=self.global_step)
                tf.summary.scalar('loss_eval/field', field_loss, step=self.global_step)
                print(f'Evaluation Loss: Detect {detect_loss}'
